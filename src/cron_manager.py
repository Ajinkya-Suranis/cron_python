...
def loop_execute_jobs(mgr_obj):
    ...
    while True:
        ...
        with mgr_obj.heap_lock:
            cron_group = h.remove_min()
        for task in cron_group["task_items"]:
            if task.schedule_units["every_seconds"] != None:
                # TODO: put an assert here
                with mgr_obj.seconds_dict_lock:
                    # The task might have been removed or rescheduled
                    # while we were waiting for lock. If so, skip that task.
                    if task.job_uuid not in mgr_obj.seconds_job_dict or \
                        mgr_obj.seconds_job_dict[task.job_uuid]["state"] == SEC_CRON_RESCHEDULING:
                        continue
...
def handle_seconds_job(mgr_obj, job_obj, gen_count):
    ...
    while True:
        ...
        job_thr.daemon = True
        job_thr.start()
        # TODO: Do the updating within a job-level lock. Probably add the code inside
        # cron_job class itself.
    ...
    def _modify_from_nonsec(self, job_obj, every_seconds, minutes, hours, dom, months):
        ...
        with self.heap_lock:
            ...
            if len(cron_group["task_items"]) == 0:
                ...
                # the job. Need to remove it from heap as well.
                self.heap.remove(cron_group)
        # TODO: Introduce a new schedule lock at cron job object level
        # to avoid the race condition of two or more threads simultaneously
        # modifying the schedule on same cron job.
